...
- **再現可能性**. これは非常に大きな概念であり、1つの章で完全に扱うことはできない。
  しかし、本章では、誰でも再現可能なアプローチ に重点を置く。
  それは、見知らぬ人、共著者、さらには将来の自分自身 であっても再現できる方法である。
...
- **整然データ**(Tidy data)(https://tidyr.tidyverse.org/articles/tidy-data.html)[https://tidyr.tidyverse.org/articles/tidy-data.html]. 私たちが扱うデータの多くは、本章で扱うデータを含め、本質的に表形式(tabular)であり、関係性(relational)をもつ。
  この特性を理解すれば、多くのデータ処理の問題はより単純で直感的に解決できるようになる。
...
SASやStataの熱心なユーザーであっても、本章は有用であると感じるかもしれない。
なぜなら、Rは複雑なデータ加工(data-munging)作業を容易に処理でき、最終的なデータを他のソフトウェアが読み取れる形式で簡単にエクスポートできるからである。例えば、`haven`パッケージの`write_dta()`関数を使用すれば、Stataのデータファイルを作成できる。

<!-- We also introduce **regular expressions**.  -->
本章では、**正規表現**(regular expressions)についても紹介する。
...
簡単に言えば、正規表現とは、一連の文字列パターンを定義し、高度な検索や検索・置換を行うための強力なツールである。
<!-- Regular expressions are very useful when working with data in many contexts.[^2] -->
正規表現は、さまざまなデータ処理の場面で非常に有用である。[^2]
...
『R for Data Science』の文字列処理と正規表現に関する章 は、本章の内容を補完する優れた入門資料となっている。
<!-- We recommend that you refer to those chapters as you work through this chapter. -->
本章を進める際には、それらの章を参照することを推奨する。
...
本章のコードでは、以下のパッケージを使用する。
本書のコードを実行するための環境設定については、セクション1.2を参照のこと。
また、本章の演習用 Quarto テンプレート は GitHub で利用可能である。
:::
